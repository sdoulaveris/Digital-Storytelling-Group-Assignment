# Lesson: Digital Storytelling
# Transmedia Document

### First and Last Name: Bizremis Anastasis Gavridakis Adonis Spiros Doulaveris kai h lidia mas (all members of the team)
### University Registration Number: 
dpsd19033
dpsd19013 
dpsd19041 
dpsd19079
### GitHub Personal Profile: 
[spiros](https://github.com/sdoulaveris)
[adonis](https://github.com/tonygavridakis) 
[h lidia mas](https://github.com/lydiaznt)
[anastasis](https://github.com/BizremisA)
* 1.1. What is the project about?
* Το προτζεκτ μας ασχολείται με τις ζωές δύο νέων οι οποίες είναι διαμετρικά αντίθετες. Μιλάει για την διαφορά ανάμεσα σε μια επικίνδυνη και μια ασφαλή ζωή.
* 1.2. What is the projects core ?
* Το βασικό νόημα της ταινίας είναι το πόσο εύκολα ανατρέπονται τα προκαθορισμένα σχέδια όταν μπαίνουν στην μέση απρόοπτα γεγονότα 
* 1.8. Is it a fictional, a non-fiction or a mixed project ?
* Πρόκειται για να έναν ρεαλιστικό κόσμο
*  1.4. What is its fundamental purpose?
* Is it to entertain, to teach or to inform ? Is it to market a product ?
* Ο κύριος στόχος είναι να ψυχαγωγήσει αλλά παράλληλα να κάνει το κοινό να αναλογιστεί την πραγματική αξία της ζωής 

* 2. Narrative

* 2.1. What are the narrative elements (such as plot, theme, characters, etc.) of the project ?
* Η πλοκή εκτυλίσσεται στην Αθήνα και ακολουθεί την καθημερινότητα δύο εντελώς διαφορετικών ανθρώπων. Ο πρώτος, ο Μike μένει σε μια κακόφημη περιοχή ,όπου βγάζει τα προς το ζην κάνοντας παράνομες δουλειές και συναναστρέφεται με τον υπόκοσμο. Επειδή χρωστάει στον προμηθευτή του πολλά λεφτά, έχει εξαπολυθεί ένα ανθρωποκυνηγητό εναντίον του. Στο μεταξύ ο δεύτερος, ο Ανέστης, ζει μια ήσυχη φοιτητική ζωή αποφεύγοντας διαπληκτισμούς και εντάσεις, γεμάτη στόχους και οράματα για το μέλλον του. Οι συγκυρίες θα οδηγήσουν σε μια απρόσμενη κατάσταση...
* 2.2. What would be the summary of its storyline?
* Ακόμα και όταν κάνεις τα πράγματα όπως πρέπει η τύχη έχει άλλα σχέδια
* 2.3. What is the timeframe of the story?
* Το χρονικό πλαίσιο της πλοκής δεν καθορίζεται από τους διαλόγους, αλλά μάλλον τα στοιχεία που χρησιμοποιούνται από τους χαρακτήρες, όπως αυτοκίνητα, ενδυμασία, και τεχνολογία εποχής. Υπονοείται λοιπόν ότι το χρονικό πλαίσιο είναι στις αρχές του 2000.
* 2.4. What are the major events or challenges offered by the narrative?
* Το κομβικό γεγονός συμβαίνει όταν ο Μike βρίσκεται σε οικονομικό και αδιέξοδο καλείται να πληρώσει το τίμημα των αποφάσεων του και ο Ανέστης βρίσκεται στο λάθος μέρος την λάθος στιγμή.
* 2.5. Does the project utilize gaming elements? Does the project involve winning or losing?
* Οχι
* 2.6. What are the strategies for expanding the narrative?
* Ξεκινάμε με exposition της καθημερινότητας των δύο χαρακτήρων με απουσία υπερβολικού διαλόγου αφήνοντας τις πράξεις τους και τις συνήθειες τους να προχωρήσουν την πλοκή του έργου ενώ στο τέλος κάνουμε χρήση μιας ανατροπής. 
* 2.7. Are negative capability and migratory cues included?
* Ναι 
* 2.8. Is it possible to identify intermedial texts in the story?
* Όχι
* WORD BUILDING
* 3.1. When the story occurs?
* Δεκαετία 2000
* 3.2. Which is the central world where the project is set?
* στις γειτονιές τις Αθήνας, 
* 3.3. Is it a fictional world, the real world or a mixture of both?
* Πραγματικός κόσμος
* 3.4. How it is presented geographically?
* Το περιβάλλον παρουσιάζεται καθώς εκτυλίσσεται η ιστορία
* 3.5. How the world looks?
* Το τοπίο στην ζωή του Μike είναι κακόφημες γειτονίες και του Σπύρο ποιο είναι πιο οργανωμένος και ταξικά ανεβασμένος
* 3.6. What challenges, dangers, or delights are inherent to this world?
* Παρατηρούμε τους κίνδυνους που έχει ο υπόκοσμος (όπλα, ναρκωτικά κατάχρηση, κυκλώματα)
* 3.7. Is the storyworld big enough to support expansions?
* ναι
* 4. Characters
* 4.1. Who are the primary and secondary characters of the story?
* Οι βασικοί χαρακτήρες είναι ο χάρης και ο σπύρος, δευτερεύοντες το αφεντικό και ο δολοφόνος,
* 4.2. Does the project have any spin-offs15? If so, who are the spin-offs’ protagonists?
* όχι
* 4.3. Can the storyworld be considered a primary character of its own?
* όχι
* 4.4. Can the audience be considered a character as well?
* όχι
* 4.5. Are there non-player characters (NPCs) in this project? If so, who are they and what kind of role do they play (allies, adversaries, helper figures, etc.)?
* οι παρέες του κάθε χαρακτήρα που τον πλαισιώνει και κάνει τον κόσμο πιο πιστευτό
* 5. Extensions
* 5.1. How many extensions does the project have?
* το κόμικς και το digital booklet και τα bloopers
* 5.2. Are the extensions adaptations (the intersemiotic translation from one system to another) or expansions of the narrative through various media?
* είναι οπτικά
* 5.3. Is each extension canonical? Does it enrich the story?
* το booklet
* 5.4. Does each extension maintain the original characteristics of the world?
* ναι, στο booklet αιτιολογείται ο χώρος και οι παρέες των ηρώων
* 5.5. Does each extension answer questions left previously unanswered?
* όχι
* 5.6. Does each extension raise new questions?
* όχι
* 5.7. Do the extensions open up new possibilities for additional expansion?
* όχι
* 5.8. Do the extensions have the ability to spread the content and also to provide the possibility to explore the narrative in-depth?
* ναι
* 6. Media platforms and genres
* 6.1. What kind of media platforms (film, book, comics, games, and so forth) are involved in the project?
* το κόμικ
* 6.2. Which devices (computer, game console, tablet, mobile phone, etc.) are required by the project?
* θα είναι σε ηλεκτρονική μορφή οπότε χρειάζεται υπολογιστή ή κινητό. 
* 6.3. How each platform is participating and contributing to the whole project? What are their functions in the project?
* βλέπει την ταινία από διαφορετικό καλλιτεχνικό στυλ και οπτική
* 6.4. What are the distinctive characteristics of each media platform?
* η ταινία δίνει παραπάνω λεπτομέρεια και συνδυάζει ήχο εικόνα και κίνηση των το κόμικ έχει ένα ξεχωριστό στυλ και έχει στοιχεία character design. Επίσης οργανώνει τα στοιχεία της κάθε σκηνής με διαφορετικό τρόπο λόγω περιορισμού frame
* 6.5. Identify problems that are specific of each medium.
* Στην ταινία πρέπει να συντονιστούν πολλοί διαφορετικοί άνθρωποι με άλλα προγράμματα και εξειδικεύσεις ενώ στο κόμικ πρέπει με λίγες εικόνες και λόγια να επικοινωνήσεις όλα τα επίπεδα της ιστορίας.
* 6.6. ls each medium really relevant to the project?
* Περιγράφουν την ίδια ιστορία.
* 6.7. What is the roll-out strategy to release the platforms?
* στο youtube και σε όλα τα social media.
* 6.8. Which genres (action, adventure, detective, science fiction, fantasy, and so forth) are present in the project?
* δράση και δράμα εποχής.
* 7. Audience and market
* 7.1. What is the target audience of the project? Who is the intended VUP?
* απευθύνεται σε άτομα άνω των 15
* 7.2. What kind of “viewers” (real-time,reflective, and navigational) does the project attract?
* navigational.
* 73. What kind of entertainment does the target audience enjoy?
* Άτομα που τους αρέσουν οι ταινίες δράσης και το σασπένς
* 7.4. What kind of technology/devices are people in this group involved with?
* χρησιμοποιεί τις σύγχρονες τεχνολογίες
* 7.5. Why does this project appeal to them?
* έχει δράση, αναπάντεχη πλοκή ευρηματική σκηνοθεσία και σενάριο που κόβει την ανάσα. 
* 7.6. Do other projects like this exist? Do they succeed in achieving their purpose?
* ναι, είναι διαδεδομένο είδος ταινιών με πολλές επιτυχίες.
* 7.7. What is the project's business model?
* ήταν ομαδική εργασία, συμμετείχαν όλοι
* 7.8. Revenue-wise, was the project successful? Why?
* δεν το έχουμε δείξει ακόμα
* 8. Engagement
* 8.1. Through what point of view (PoV) does the VUP experience this world: first-person, second-person,
third-person, or a mixture of them?
* To vup βλέπει την ιστορία να εκτυλίσσεται παράλληλα με τους πρωταγωνιστές (third person experience)
* 8.2. What role does the VUP play in this project?
* Παρατηρητής
* 8.3. How the project keeps the VUP engaged?
* Λόγω της δράσης και των αναπάντεχων γεγονότων
* 8.4. What are the mechanisms of interaction in this project?
* Δεν έχει
* 8.5. Is there also participation involved in the project? If so, how can the VUP narticinnts in tha nanan evietam
* Δεν έχει
* 8.6. Does the project work as cultural attractor/activator?
* Ναι διότι τα κουστούμια και το σκηνικό μεταφέρουν το vup σε εκείνη  την εποχή
* 8.7 How does the VUP affect the outcome? What do they add to the storyworld?
* δεν το επηρεάζει
* 8.8. Are there UGC related to the story (parodies, recaps, mash-ups, fan communities, etc.)?
* οχι
* 8.9. Does the project offer the VUP the possibility of immersion into the storyworld?
* όχι
* 8.10. Does the project offer the VUP the possibility to take away elements of the story and incorporate them into everyday life?
* όχι
* 8.11. ls there an important goal that the VUP is trying to accomplish in the project?
* όχι+
* 8.12. What will make the VUP want to spend time accomplishing this Goal?
* α καλά
* 8.13. What adds tension to the experience? Are there any ticking Clocks?
* ο δολοφόνος, ο καταδότης και το guest στην τελευταία πράξη
* 8.14. Is there a system of rewards and penalties?
* ισχύουν οι κανόνες της νύχτας, το δίκαιο της πυγμής και το άδικο της στιγμής.
* 9. Structure
* 9.1. When did the transmediation begin? Is it a proactive or retroactive TS project?
* Τα γεγονότα προβάλλονται με την σειρά που διαδραματίζονται
* 9.2. Is it possible to identify any consequences for the project caused by the fact that this is either a pro-active or a retroactive transmedia story?
* όχι
* 9.3. Is this project closer to a transmedia franchise, a portmanteau transmedia story, or a complex transmedia experience?
* Είναι complex transmedia experience γιατί παρακολουθούμε παράλληλα δυο κόσμους αντίθετους να συγκρούονται
* 9.4, Can each extension work as an independent entry point for the Story?
* όχι
* 9.5. What are/were possible endpoints of the project?
* προτείνουμε η ταινία να παρακολουθηθεί μέχρι το τέλος για να ληφθεί το μήνυμα που θέλουμε να προβάλουμε
* 9.6. How is the project structured? What are the major units of Organisation?
* Οι όλα τα κομμάτια της ιστορίας συζητήθηκαν από την ομάδα, το εντιτ και η ενδυματολογία έγιναν χωριστά
* 9.7. How could a mag of the story-world be presented?
* Ένας απλός χάρτης της Αθήνας
* 10. Aesthetics
* 10.1. What kinds of visuals are being used (animation, video, graphics, a mix) in the project?
* χρησιμοποιήσαμε την κάμερα ηχητικά εφέ, και σκίτσα για το κόμικ
* 10.2. Is the overall look realistic or a fantasy environment?
* ρεαλιστικό
* 10.3. Is it possible to identify specific design styles in the project?
* τις διάφορες των χρωμάτων για τον κόσμο του κάθε πρωταγωνιστή, τα διαφορετικά ενδυματολογικά στυλ για κάθε κόσμο και το μουντό ασπρόμαυρο του κόμικ.
* 10.4. How does audio work in this project? Is there ambient sound (rain, wind, traffic noises, etc.), sound effects, music, and so forth?
* οι πυροβολισμό και η μουσική που παίζει πίσω από τις σκηνές.


